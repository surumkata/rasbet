...
function check_amount(elem){
  if(elem.value<0.10 && elem.value!="" && elem.value>0){
      ...
      elem.parentElement.outerHTML += "<div>A aposta mínima é de 0.10€</div>"
  }else if(elem.value>500){
      $('#buttonApostar').attr("disabled", true);
      elem.parentElement.outerHTML += "<div>A aposta máxima é de 500€</div>"
  }
}
  ...
  // Multiple
    ...
    if(window.game_counter<=10){
      ...
      for(var i=0;i<betboxs.length;i++){
          odd = parseFloat(betboxs[i].getAttribute("data-odd"))
          if(odd < 1.10){
            $("#rowCimaNome span").text("Esta aposta múltipla não é válida, cota de um jogo tem de ser superior a 1.10")
    ...
    storage_change_multiple(home,away,bet,odd)
      ...
      alert("Apenas 10 seleções podem ser feitas no modo múltiplo");
      // disable bet button
      $('.buttonApostar').prop('disabled', true);
    }
...
function button_handler(elem){
  ...
  if(checked == "false"){
    ...
    slip.innerHTML += '<div class="betbox" id='+slip_id+' data-odd='+odd+' data-oddType='+odd_type+' data-home='+str_home+' data-away='+str_away+' data-bet='+str_bet+' ><div class="betboxHeader"><button onclick="delete_selection('+str_slip_id+');" ><span>&times;</span></button><label>' + home + '-' + away + '<br>Resultado: ' + bet + '<br>Cota '+odd+' </div></label></div>';
    ...
    if(game_on_slip != null && typeof(game_on_slip) != 'undefined'){
      ...
      if(window.game_counter==0){
        let slip = document.querySelector('.slip');
        slip.innerHTML += '<div class="betbox-msg" id="slipbodyMsg">Faz a tua primeira aposta!</div>'
      }
